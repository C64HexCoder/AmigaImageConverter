...
namespace Retro
{
    public partial class Palette : UserControl
    {
        ...
        [Category("C64 / Amiga Config")]
        [Description("בחר האם הפלטה מתנהגת כמו אמיגה או כמו קומודור 64")]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public HardwareType CurrentHardware { get; set; } = HardwareType.Amiga;
        ...
        public bool RGBVisible
        {
            set
            {
                ...
                Controls.Remove(redNum);
                greenNum.Visible = value;
                Controls.Remove(greenNum);
                blueNum.Visible = value;
                Controls.Remove(blueNum);
                RedLbl.Visible = value;
                Controls.Remove(RedLbl);
                GreenLbl.Visible = value;
                Controls.Remove(GreenLbl);
                BlueLbl.Visible = value;
                Controls.Remove(BlueLbl);
            }
        ...
        public byte SelectedColorIdx
        {
            ...
            set
            {
                ...
                if (colorPanel.Controls.Count > 0)
                {
                    ...
                    DrawBorder((ColorBox)colorPanel.Controls[_SelectedColorIdx], colorPanel.Controls[_SelectedColorIdx].BackColor, 4);

                    DrawBorder((ColorBox)colorPanel.Controls[value], Color.Black, 4);
                }
                ...
                // אם אנחנו בקומודור, ננעל את פאנל ה-RGB שלא יהיה ניתן לשינוי
                if (CurrentHardware == HardwareType.Commodore64)
                {
                    RGBPanel.Enabled = false; // נועל את התיבות של ה-NumericUpDown
                }
                else
                {
                    RGBPanel.Enabled = true;
                }
        ...
        public bool SelectedColorBox
        {
            ...
            get
            {
                return selectedColorPb.Visible;
            }
        }
        ...
        private void colorPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            int index = (int)(Math.Log(Colors.Length) / Math.Log(2));
        ...
        private void ColorMouseClick(object? sender, MouseEventArgs e)
        {
            ...
            // חסימת קליק ימני לקומודור 64 - לא מאפשרים בחירת צבע חופשי מהמחשב!
            if (e.Button == MouseButtons.Right && CurrentHardware == HardwareType.Commodore64)
            {
                // בקומודור אי אפשר לשנות את ה-RGB של הפלטה המקורית, לכן פשוט נתעלם או שנשמיע ביפ
                SystemSounds.Beep.Play();
                return;
            }


            if (e.Button == MouseButtons.Right && CurrentHardware == HardwareType.Amiga && !(pickedColor.Tag.ToString() == "0"))
            {
            ...
            SelectedColorEventArgs args = new SelectedColorEventArgs { Color = SelectedColor, colorIndex = index };
        ...
        {
            byte i = 0;
            while (i < Colors.Length && Colors[i].ToArgb() != color.ToArgb())
            {
                i++;
            }
            return i;
